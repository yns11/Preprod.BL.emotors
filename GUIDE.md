...
Sur ces deux vues, le bandeau de contexte et l'en-tête de l'application sont
**masqués** : toute la hauteur d'écran va à l'image en cours de relecture.
...
L'écran indique d'où vient la reconnaissance : « reconnu (code ✓) » ou
« reconnu (nom ✓) ». Une seule différence avec la saisie unitaire : le mode lot
ne relance pas le modèle avec du contexte (un appel couvre plusieurs BL) ; la
liste des tiers est donc injectée **dès le premier appel** dans le prompt.
