...
| Composant | Rôle |
|---|---|
| **App Création** | Terrain (mobile/tablette) : un BL à la fois, plusieurs photos attachées, champs pré-remplis par IA. Notifie Teams à chaque **nouvelle réception**. |
| **App Administration** | Poste fixe : pilotage (tableau de bord, KPI), **rapports d'activité PDF**, **archivage par lots** (PDF multipage), vues BL/DESADV/Rapprochement, référentiels, rôles. Notifie Teams au **passage EDI NOK → OK**. |
...
| Variable | App | Valeur |
|---|---|---|
| `BL_ENVIRONMENT` | les deux | `local`, `dev`, `rec`, `pre-prod` ou `prod` |
...
# 4. Archivage par lots d'anciens BL

## Où, et pourquoi là

L'archivage de masse est dans l'app **Administration**, pas dans l'app
Création. La raison est ergonomique : relire une page de BL et corriger les
champs détectés suppose d'afficher la page **en grand**. L'app Création est
utilisée au quai, sur smartphone et tablette, où cette largeur n'existe pas ;
elle conserve donc son parcours d'origine — un BL à la fois, plusieurs photos
attachées comme pages.

Deux vues, selon le sens et le rôle :

| Vue | Objet | Rôles |
|---|---|---|
| **Achat ▸ Archivage par lots** | archivage de réceptions | APPROS, ADMIN_METIER |
| **Vente ▸ Archivage par lots** | archivage d'expéditions | ADV, ADMIN_METIER |

Sur ces deux vues, le fil d'Ariane cède la place à un titre compact : la
hauteur d'écran gagnée va à l'image en cours de relecture, qui occupe **70 %
de la largeur utile**. Elle est affichée dans une visionneuse offrant
**zoom, ajustement et rotation**, tous côté navigateur — zoomer ne relance pas
le script et ne fait donc rien perdre de la saisie en cours.

## Le parcours

```
Étape 1  Dépôt d'UN PDF (jusqu'à 200 pages) — chaque page = un BL
             │  rasterisation en JPEG, une image par page
             ▼
Étape 2  Vérification page par page
             ├── à droite : l'image de la page, en grand
             ├── à gauche : numéro, date, tiers — détectés par l'IA, corrigeables
             ├── « Valider et continuer »  -> le BL rejoint la liste d'attente
             ├── « Ignorer cette page »    -> page écartée (séparateur, verso…)
             ├── « Page précédente »       -> annule la décision précédente
             └── « Terminer ici »          -> passe au récapitulatif
             ^
             |  EN ARRIÈRE-PLAN : les paquets de pages suivants sont déjà
             |  envoyés au modèle pendant que l'opérateur vérifie
Étape 3  Récapitulatif -> enregistrement de tous les BL retenus
```

**Pourquoi c'est fluide.** Chaque page est analysée **seule**, mais **en
avance** : pendant que l'opérateur vérifie la page N, les pages N+1 et N+2 sont
déjà parties au modèle. Le clic « page suivante » n'attend donc pratiquement
jamais.

> Une version antérieure envoyait **4 BL en un seul appel** pour diviser le
> coût. La qualité de lecture s'en trouvait nettement dégradée — le modèle
> mélangeait des champs entre bordereaux voisins. Ce mode a été **retiré** :
> une page = un appel, par la procédure d'extraction unitaire complète.

| Réglage (`app.yaml` de l'app Administration) | Défaut | Effet |
|---|---|---|
| `BL_LLM_ENDPOINT` | *(nom de l'endpoint)* | vide = saisie 100 % manuelle |
| `BL_LOT_MAX_PAGES` | `200` | pages acceptées dans un même PDF |
| `BL_LOT_PAGES_AVANCE` | `2` | pages pré-analysées en arrière-plan (0 = aucune) |

## Rapprochement au référentiel

Les valeurs lues sont confrontées au référentiel **exactement comme en saisie
unitaire** :

1. le **code tiers** (`S-000000` / `C-000000`) prime sur la raison sociale ;
2. à défaut, rapprochement sur la raison sociale (exact, sous-chaîne, puis
   proximité) ;
3. le tiers reconnu, ses **numéros de BL connus (DESADV)** sont confrontés au
   numéro lu : en cas de correspondance à un préfixe/suffixe près, c'est la
   version du **référentiel** qui est retenue — l'ERP se trompe moins que
   l'OCR. L'écran affiche alors les deux (« lu : … — retenu : … »).

C'est **exactement** `extraction.extraire_infos_bl`, la fonction qu'utilise
l'app Création : mêmes passes de raffinement (réinterrogation du modèle avec la
liste des tiers si le tiers n'est pas reconnu, puis avec les BL du tiers
reconnu). Aucune différence de traitement entre une saisie unitaire et une page
de lot. L'écran indique d'où vient la reconnaissance : « reconnu (code ✓) » ou
« reconnu (nom ✓) ».

## Garde-fous à l'étape 3

Avant enregistrement, chaque BL retenu est contrôlé : numéro manquant, numéro
**en double dans le lot**, numéro **déjà présent en base**, tiers non
renseigné. Les lignes en anomalie sont affichées et **exclues** de
l'enregistrement — le reste du lot part quand même. Chaque BL est une
...
Si l'IA est indisponible, le parcours reste utilisable : les champs arrivent
vides et sont saisis à la main.
...
5. **Principaux tiers** (top 10) et **tiers à surveiller** — classés par
   **part des EDI NOK**, c'est-à-dire le poids du tiers dans le *total* des
   anomalies de la période, et non son taux d'échec propre. Un tiers à 100 %
   d'échec sur 2 réceptions pèse ainsi moins qu'un tiers à 10 % sur 400 : le
   classement désigne où porter l'effort, pas qui a le pire ratio.
...
|---|---|
| Un BL archivé récemment est introuvable | Les filtres de période portent par défaut sur la **date d'opération**. Pour un archivage saisi hier sur un BL ancien, basculer « La période porte sur » ▸ **Date de saisie** dans le panneau Filtres. |
