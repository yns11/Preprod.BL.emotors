...
from bl_core import (extraction, notifications, pdf_bl, pdf_lot, rapports, rbac,
                     repository, ui)
...
NAVIGATION = [
    ...
    ("Achat", [("📥", "BL réception"), ("📡", "DESADV achat"),
               ("⚖️", "Rapprochement achat"), ("🗃️", "Archivage réception")]),
    ("Vente", [("📤", "BL expédition"), ("📡", "DESADV vente"),
               ("⚖️", "Rapprochement vente"), ("🗃️", "Archivage expédition")]),
...
LIBELLES_VUES = {"Rapprochement achat": "Rapprochement BL / DESADV",
                 ...
                 "Archivage réception": "Archivage par lots",
                 "Archivage expédition": "Archivage par lots"}
# Vues qui exploitent toute la largeur : ni bandeau de contexte, ni fil
# d'Ariane, pour laisser la hauteur d'écran à l'image en cours de relecture.
VUES_PLEIN_ECRAN = {"Archivage réception", "Archivage expédition"}
...
# --- Archivage par lots : état de session, partagé par la vue et la nav ---
ETAPES_LOT = {1: "Dépôt du PDF", 2: "Vérification par page", 3: "Récapitulatif"}
CLES_LOT = ("lot_etape", "lot_pages", "lot_index", "lot_retenus", "lot_ignores",
            "lot_prechargeur", "lot_resultats", "lot_pdf_nom", "lot_seq")


def _lot_fermer_prechargeur() -> None:
...
def _lot_reinitialiser(garder_seq: bool = False) -> None:
    _lot_fermer_prechargeur()
    seq = st.session_state.get("lot_seq", 0)
    for cle in CLES_LOT:
        st.session_state.pop(cle, None)
    st.session_state.lot_seq = seq + 1 if garder_seq else seq
...
with st.sidebar:
    ui.afficher_logo()
    # Contexte (identité, environnement, rôles) en haut à gauche : il ne
    # consomme plus une bande horizontale sur chaque vue.
    ui.barre_contexte_laterale(utilisateur, SETTINGS.environment,
                               CTX_RBAC["roles"])
    st.divider()
...
# =====================================================================
# VUE « ARCHIVAGE PAR LOTS » — un PDF, une page = un BL
# =====================================================================
# L'app Création reste dédiée au terrain (un BL, plusieurs photos, mobile).
# L'archivage de masse est ici : l'écran large permet d'afficher la page en
# grand pendant la relecture, ce qui est la contrainte réelle de cette tâche.
def _lot_referentiel(type_tiers: str, sens: str):
    """Même contexte de rapprochement qu'en saisie unitaire : liste des tiers,
    adresses de sites, et numéros de BL connus par tiers (DESADV)."""
    try:
...
def vue_archivage_lot(type_op: str) -> None:
    tiers_libelle = repository.libelle_tiers(type_op)
    ...
    etape = st.session_state.setdefault("lot_etape", 1)
    st.session_state.setdefault("lot_pages", [])
    st.session_state.setdefault("lot_index", 0)
    st.session_state.setdefault("lot_retenus", [])
    st.session_state.setdefault("lot_ignores", [])
    st.session_state.setdefault("lot_resultats", {})
    st.session_state.setdefault("lot_prechargeur", None)

    # -----------------------------------------------------------------
    # ÉTAPE 1 — Dépôt du PDF
    # -----------------------------------------------------------------
    if etape == 1:
        ui.afficher_etapes(1, ETAPES_LOT)
        st.caption(
            f"Déposez le PDF des anciens BL {'expédition' if est_vente else 'réception'} "
            f"à archiver. **Chaque page est traitée comme un BL distinct** "
            f"(jusqu'à {SETTINGS.lot_max_pages} pages). Vous validerez ou "
            "écarterez chaque page à l'étape suivante.")
        fichier = st.file_uploader(
            "Fichier PDF du lot", type=["pdf"],
        ...
        if fichier is not None and not st.session_state.lot_pages:
            ...
            if not octets:
                st.warning("Le fichier n'a pas été transmis (connexion "
                           "interrompue ?). Redéposez-le.")
            else:
                barre = st.progress(0.0, text="Lecture du PDF…")
                try:
                    pages = pdf_lot.rasteriser(
                        octets,
                        progression=lambda i, t: barre.progress(
                            i / t, text=f"Conversion de la page {i}/{t}…"))
                except Exception as exc:
                    barre.empty()
        ...
        if st.session_state.lot_pages:
            ...
            apercu = st.columns(6)
            for position, page in enumerate(st.session_state.lot_pages[:6]):
                apercu[position].image(page, caption=f"p.{position + 1}",
                                       width="stretch")
            if total > 6:
                st.caption(f"Aperçu des 6 premières pages sur {total}.")
            actions = st.columns([2, 2, 4])
            if actions[0].button("▶️ Commencer la vérification", type="primary",
                                 width="stretch"):
                st.session_state.lot_etape = 2
                st.rerun()
            if actions[1].button("🗑️ Déposer un autre PDF", width="stretch"):
                _lot_reinitialiser(garder_seq=True)
                st.rerun()
        return

    # -----------------------------------------------------------------
    # ÉTAPE 2 — Vérification page par page (image en grand à droite)
    # -----------------------------------------------------------------
    if etape == 2:
        ...
        if not pages_lot:
            st.session_state.lot_etape = 1
            st.rerun()
        if index >= total:
            st.session_state.lot_etape = 3
            st.rerun()

        tous_tiers, referentiel = _lot_referentiel(type_tiers, sens)
        if st.session_state.lot_prechargeur is None:
            st.session_state.lot_prechargeur = pdf_lot.Prechargeur(
                pages_lot, tiers_libelle.lower(), referentiel)
        prechargeur = st.session_state.lot_prechargeur
        prechargeur.amorcer(index)          # entretient l'avance

        gauche, droite = st.columns([1, 1.45], gap="medium")

        with gauche:
            ui.afficher_etapes(2, ETAPES_LOT)
            st.progress(
                index / total,
            ...
            if not prechargeur.actif:
                ...
                st.warning("Analyse IA indisponible pour cette page : saisie "
                           "manuelle.")
                with st.expander("Détail technique"):
                    st.code(erreur_ia)
            elif any(infos.get(c) for c in ("numero_bl", "tiers", "code_tiers",
                                            "date")):
                analysees, _t = prechargeur.avancement()
                st.success("🔎 Champs détectés — vérifiez et corrigez si besoin "
            ...
            if infos.get("numero_bl_lu"):
                # Le numéro a été corrigé depuis les DESADV du tiers reconnu.
                st.caption(f"🔎 lu : « {infos['numero_bl_lu']} » — retenu depuis "
                           f"l'avis d'expédition : « {infos['numero_bl']} »")
            elif infos.get("numero_bl"):
                st.caption(f"🔎 détecté : « {infos['numero_bl']} »")

            date_detectee = _parse_date_lot(infos.get("date", ""))
            date_bl = st.date_input(
                "Date d'expédition *" if est_vente else "Date de réception *",
                value=date_detectee or datetime.date.today(),
                key=f"lot_date_{index}")
            if infos.get("date"):
                st.caption(f"🔎 détecté : « {infos['date']} »")

            reconnu = infos.get("tiers_reconnu")
            filtre = st.text_input("Filtrer la liste", value="",
                                   key=f"lot_filtre_{index}",
            ...
            fournisseur = st.selectbox(f"{tiers_libelle} *", options=options,
                                       ...
                                       key=f"lot_tiers_{index}")
            detecte = " / ".join(x for x in (infos.get("code_tiers"),
                                             infos.get("tiers")) if x)
            if detecte:
                if reconnu:
                    marque = ("code ✓" if infos.get("fiabilite_tiers") == "code"
                              else "nom ✓")
                    st.caption(f"🔎 détecté : « {detecte} » — reconnu "
                               f"({marque}) : {reconnu}")
                else:
                    st.caption(f"🔎 détecté : « {detecte} » — non trouvé au "
                               "référentiel, choisissez manuellement.")
        ...
        st.divider()
        actions = st.columns([2, 1.6, 1.4, 2.4])
        if actions[0].button("✅ Valider et continuer", type="primary",
                             width="stretch"):
            if not numero.strip():
                st.error("Le numéro de BL est obligatoire pour retenir cette page.")
            elif not fournisseur:
                st.error(f"Le {tiers_libelle.lower()} est obligatoire pour "
                         "retenir cette page.")
            else:
        ...
        if actions[1].button("⏭️ Ignorer cette page", width="stretch",
                             help="La page n'est pas un BL, ou ne doit pas être "
                                  "archivée."):
            st.session_state.lot_ignores.append(index)
            st.session_state.lot_index = index + 1
            prechargeur.amorcer(index + 1)
            st.rerun()
        if actions[2].button("⬅️ Page précédente", width="stretch",
                             disabled=index == 0):
            # Revenir en arrière annule la décision prise sur cette page.
            precedente = index - 1
            st.session_state.lot_retenus = [
                bl for bl in st.session_state.lot_retenus
                if bl["page"] != precedente]
            st.session_state.lot_ignores = [
                p for p in st.session_state.lot_ignores if p != precedente]
            st.session_state.lot_index = precedente
            st.rerun()
        if actions[3].button("⏩ Terminer ici et passer au récapitulatif",
                             width="stretch",
                             help=f"Les {total - index} page(s) restantes seront "
                                  "ignorées."):
            st.session_state.lot_etape = 3
            st.rerun()
        return
    ...
    # -----------------------------------------------------------------
    ui.afficher_etapes(3, ETAPES_LOT)
    _lot_fermer_prechargeur()
    ...
    controles = pdf_lot.controler_lot(
        retenus, lambda numero: repository.numero_bl_disponible(numero, type_op))
    ...
    st.dataframe(
        [{"Page": bl["page"] + 1, "N° BL": bl["numero"],
          "Date": bl["date_reception"], tiers_libelle: bl["fournisseur"],
          "Anomalie": bl["anomalie"] or "—"} for bl in controles],
        hide_index=True, width="stretch",
        height=min(420, 60 + 36 * max(1, len(controles))))

    if anomalies:
        st.warning(f"{len(anomalies)} ligne(s) en anomalie ne seront **pas** "
                   "enregistrées. Revenez à la vérification pour les corriger, "
                   "ou validez le reste du lot.")

    actions = st.columns([2, 2, 4])
    if actions[0].button("⬅️ Revenir à la vérification", width="stretch"):
        st.session_state.lot_index = min(st.session_state.lot_index,
                                         max(0, total - 1))
        st.session_state.lot_etape = 2
        st.rerun()
    if actions[1].button(f"💾 Enregistrer les {len(valides)} BL", type="primary",
        ...
        for position, bl in enumerate(valides, start=1):
            try:
                ...
                repository.inserer_bl(
                    ...
                    nom_fournisseur=bl["fournisseur"],
                    statut_bl=repository.STATUT_OK, type_operation=type_op,
                    utilisateur=utilisateur, date_reception=bl["date_reception"])
                repository.enregistrer_page(
                    id_bl, 0, st.session_state.lot_pages[bl["page"]])
                repository.finaliser_bl(id_bl, 1, utilisateur)
                succes.append(bl["numero"])
            except Exception as exc:
                # Chaque BL est une transaction indépendante : un échec isolé
                # n'annule pas le reste du lot.
                logger.warning("Archivage du BL %s en échec : %s", bl["numero"],
                               exc, exc_info=True)
...
def render_vue_courante() -> None:
    if PLEIN_ECRAN:
        st.markdown(f"#### 🗃️ Archivage par lots · {section.lower()}")
    ...
    elif vue == "Archivage réception":
        vue_archivage_lot(repository.TYPE_ARCHIVAGE_RECEPTION)
    elif vue == "Archivage expédition":
        vue_archivage_lot(repository.TYPE_ARCHIVAGE_EXPEDITION)
