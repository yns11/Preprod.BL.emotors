...
Wizard en 4 étapes, avec pré-remplissage assisté par IA. Deux parcours, selon
le type d'opération choisi à l'étape 1 :

**Saisie unitaire** (nouvelle réception / nouvelle expédition)
  1. Type d'opération
  2. Numérisation des pages (photo)
  3. Informations du BL, pré-remplies par extraction de l'image (LLM vision)
     puis rapprochées des référentiels
  4. Récapitulatif et enregistrement

**Archivage en lot** (anciens BL réception / expédition)
  1. Type d'opération
  2. Dépôt d'un PDF multipage — une page = un BL
  3. Vérification page par page : l'image à gauche, les champs détectés à
     droite ; « Valider et continuer » ou « Ignorer cette page ». Les pages
     suivantes sont analysées par paquets EN ARRIÈRE-PLAN pendant la
     vérification, de sorte que le passage à la page suivante n'attend pas
     le modèle.
  4. Récapitulatif du lot et enregistrement de tous les BL retenus
"""
...
from bl_core import (extraction, images, notifications, pdf_lot, rbac,
                     repository, ui, validation)
...
# L'archivage suit le même nombre d'étapes, avec des intitulés propres au lot.
NOMS_ETAPES_LOT = {1: "Type d'opération", 2: "Dépôt du PDF",
                   3: "Vérification page par page", 4: "Récapitulatif du lot"}
...
# --- État de l'archivage en lot ---
st.session_state.setdefault("lot_pages", [])          # images des pages du PDF
st.session_state.setdefault("lot_index", 0)           # page en cours de revue
st.session_state.setdefault("lot_retenus", [])        # BL validés, en attente
st.session_state.setdefault("lot_ignores", [])        # index des pages écartées
st.session_state.setdefault("lot_prechargeur", None)  # threads d'analyse
st.session_state.setdefault("lot_resultats", {})      # BL enregistrés / échecs

CLES_LOT = ("lot_pages", "lot_index", "lot_retenus", "lot_ignores",
            "lot_prechargeur", "lot_resultats", "lot_pdf_nom", "lot_seq")
...
def _fermer_prechargeur() -> None:
    """Libère les threads d'analyse (idempotent)."""
    prechargeur = st.session_state.get("lot_prechargeur")
    if prechargeur is not None:
        prechargeur.fermer()
        st.session_state.lot_prechargeur = None
...
def est_archivage(type_op: str) -> bool:
    return type_op in (repository.TYPE_ARCHIVAGE_RECEPTION,
...
MODE_LOT = est_archivage(donnees.get("type_operation", ""))

etape = st.session_state.etape
if etape in NOMS_ETAPES:
    ui.afficher_etapes(etape, NOMS_ETAPES_LOT if MODE_LOT else NOMS_ETAPES)
...
if etape == 1:
    ...
    if est_archivage(type_op):
        st.caption("Étape suivante : déposez le **PDF** des anciens BL — une "
                   "page par bordereau, jusqu'à "
                   f"{SETTINGS.lot_max_pages} pages. Vous vérifierez ensuite "
                   "chaque page, l'IA les analysant en avance en arrière-plan.")
    else:
    ...
    if st.button("Suivant ➡️", type="primary", width="stretch"):
        if type_op != donnees.get("type_operation"):
            ...
            reinitialiser_lot()          # changer de parcours repart à zéro
        donnees["type_operation"] = type_op
        aller_a(2)

# =====================================================================
# ÉTAPE 2 (LOT) — Dépôt du PDF multipage
# =====================================================================
elif etape == 2 and MODE_LOT:
    type_op = donnees["type_operation"]
    tiers = repository.libelle_tiers(type_op)
    st.caption(
        f"Déposez le PDF des anciens BL {'expédition' if type_op == repository.TYPE_ARCHIVAGE_EXPEDITION else 'réception'} "
        f"à archiver. **Chaque page est traitée comme un BL distinct** "
        f"(jusqu'à {SETTINGS.lot_max_pages} pages). Vous validerez ou écarterez "
        "chaque page à l'étape suivante.")

    fichier = st.file_uploader("Fichier PDF du lot", type=["pdf"],
                               key=f"lot_pdf_{st.session_state.get('lot_seq', 0)}")
    if fichier is not None and not st.session_state.lot_pages:
        octets = fichier.getvalue()
        if not octets:
            st.warning("Le fichier n'a pas été transmis (connexion interrompue ?). "
                       "Redéposez-le.")
        else:
            barre = st.progress(0.0, text="Lecture du PDF…")
            try:
                pages = pdf_lot.rasteriser(
                    octets,
                    progression=lambda index, total: barre.progress(
                        index / total, text=f"Conversion de la page {index}/{total}…"))
            except Exception as exc:
                barre.empty()
                st.error(f"Lecture impossible : {exc}")
            else:
                barre.empty()
                st.session_state.lot_pages = pages
                st.session_state.lot_pdf_nom = fichier.name
                st.session_state.lot_index = 0
                st.session_state.lot_retenus = []
                st.session_state.lot_ignores = []
                _fermer_prechargeur()
                st.rerun()

    if st.session_state.lot_pages:
        total = len(st.session_state.lot_pages)
        poids = sum(map(len, st.session_state.lot_pages)) / 1024 / 1024
        st.success(f"📄 **{total} page(s)** lues depuis "
                   f"« {st.session_state.get('lot_pdf_nom', 'le PDF')} » "
                   f"· {poids:.1f} Mo d'images")
        ui.afficher_miniatures(st.session_state.lot_pages[:8])
        if total > 8:
            st.caption(f"Aperçu des 8 premières pages sur {total}.")
        if st.button("🗑️ Déposer un autre PDF", width="stretch"):
            reinitialiser_lot()
            st.session_state.lot_seq = st.session_state.get("lot_seq", 0) + 1
            st.rerun()
    ...
    if col_suiv.button("Suivant ➡️", type="primary", width="stretch",
                       disabled=not st.session_state.lot_pages):
        aller_a(3)

# =====================================================================
# ÉTAPE 3 (LOT) — Vérification page par page
# =====================================================================
elif etape == 3 and MODE_LOT:
    type_op = donnees["type_operation"]
    tiers = repository.libelle_tiers(type_op)
    ...
    pages_lot = st.session_state.lot_pages
    index = st.session_state.lot_index
    total = len(pages_lot)
    ...
    try:
        ...
        tous_tiers = []
        st.error(f"Impossible de charger la liste des {tiers.lower()}s : {exc}")

    # --- Préchargeur : créé une seule fois, conservé entre les reruns -------
    if st.session_state.lot_prechargeur is None:
        referentiel = extraction.Referentiel(tiers=tous_tiers)
        st.session_state.lot_prechargeur = pdf_lot.Prechargeur(
            pages_lot, tiers.lower(), referentiel)
    prechargeur = st.session_state.lot_prechargeur
    # Entretient l'avance : les paquets suivants partent dès maintenant.
    prechargeur.amorcer(index)

    st.progress((index) / total,
                text=f"Page {index + 1} sur {total} · "
                     f"{len(st.session_state.lot_retenus)} BL retenu(s), "
                     f"{len(st.session_state.lot_ignores)} page(s) ignorée(s)")

    gauche, droite = st.columns([1, 1], gap="medium")
    with gauche:
        st.image(pages_lot[index], caption=f"Page {index + 1}/{total}",
                 width="stretch")

    with droite:
        if prechargeur.actif and prechargeur.en_cours(index):
            with st.spinner("Analyse de la page par l'IA…"):
                infos = prechargeur.resultat(index)
        else:
            infos = prechargeur.resultat(index)
        erreur_ia = prechargeur.erreurs.get(index)

        if not prechargeur.actif:
            st.caption("Analyse IA non configurée : saisie manuelle.")
        elif erreur_ia:
            st.warning("Analyse IA indisponible pour cette page : saisie manuelle.")
            with st.expander("Détail technique"):
                st.code(erreur_ia)
        elif any(infos.get(c) for c in ("numero_bl", "tiers", "code_tiers", "date")):
            analysees, _t = prechargeur.avancement()
            st.success(f"🔎 Champs détectés — vérifiez et corrigez si besoin "
                       f"({analysees}/{total} pages déjà analysées).")
        else:
            st.info("Rien de détecté sur cette page : saisie manuelle, ou "
                    "« Ignorer » s'il ne s'agit pas d'un BL.")

        # Clés indexées sur la page : chaque page a ses propres widgets.
        numero = st.text_input("Numéro du BL *", max_chars=60,
                               value=infos.get("numero_bl", ""),
                               key=f"lot_num_{index}")
        if infos.get("numero_bl"):
            st.caption(f"🔎 détecté : « {infos['numero_bl']} »")

        date_detectee = _parse_date(infos.get("date", ""))
        date_bl = st.date_input(
            "Date d'expédition *" if est_vente else "Date de réception *",
            value=date_detectee or datetime.date.today(), key=f"lot_date_{index}")
        if infos.get("date"):
            st.caption(f"🔎 détecté : « {infos['date']} »")

        # Tiers : la valeur reconnue par le rapprochement est présélectionnée.
        reconnu = infos.get("tiers_reconnu")
        if not reconnu:
            reconnu, _fiab = extraction.rapprocher_tiers(
                infos.get("code_tiers"), infos.get("tiers"), tous_tiers)
        detecte = " / ".join(x for x in (infos.get("code_tiers"),
                                         infos.get("tiers")) if x)
        filtre = st.text_input("Filtrer la liste", value="", key=f"lot_filtre_{index}",
        ...
        fournisseur = st.selectbox(f"{tiers} *", options=options, index=index_tiers,
                                   placeholder="Choisir…", key=f"lot_tiers_{index}")
        if detecte:
            st.caption(f"🔎 détecté : « {detecte} »"
                       + (f" — reconnu : {reconnu}" if reconnu
                          else " — non trouvé au référentiel"))

    st.divider()
    actions = st.columns([2, 2, 1.5])
    if actions[0].button("✅ Valider et continuer", type="primary", width="stretch"):
        if not numero.strip():
            st.error("Le numéro de BL est obligatoire pour retenir cette page.")
        elif not fournisseur:
            st.error(f"Le {tiers.lower()} est obligatoire pour retenir cette page.")
        else:
            st.session_state.lot_retenus.append({
                "page": index, "numero": numero.strip(),
                "date_reception": date_bl, "fournisseur": fournisseur,
            })
            st.session_state.lot_index = index + 1
            prechargeur.amorcer(index + 1)
            st.rerun()
    if actions[1].button("⏭️ Ignorer cette page", width="stretch",
                         help="La page n'est pas un BL, ou ne doit pas être archivée."):
        st.session_state.lot_ignores.append(index)
        st.session_state.lot_index = index + 1
        prechargeur.amorcer(index + 1)
        st.rerun()
    if actions[2].button("⬅️ Page précédente", width="stretch", disabled=index == 0):
        # Revenir en arrière annule la décision prise sur la page précédente.
        precedente = index - 1
        st.session_state.lot_retenus = [
            bl for bl in st.session_state.lot_retenus if bl["page"] != precedente]
        st.session_state.lot_ignores = [
            p for p in st.session_state.lot_ignores if p != precedente]
        st.session_state.lot_index = precedente
        st.rerun()

    if st.button("⏩ Terminer ici et passer au récapitulatif", width="stretch",
                 help=f"Les {total - index} page(s) restantes seront ignorées."):
        aller_a(4)

# =====================================================================
# ÉTAPE 4 (LOT) — Récapitulatif et enregistrement du lot
# =====================================================================
elif etape == 4 and MODE_LOT:
    type_op = donnees["type_operation"]
    tiers = repository.libelle_tiers(type_op)
    retenus = st.session_state.lot_retenus
    total = len(st.session_state.lot_pages)
    _fermer_prechargeur()                    # plus rien à analyser

    st.subheader("Récapitulatif du lot")
    resultats = st.session_state.lot_resultats

    if not resultats:
        controles = pdf_lot.controler_lot(
            retenus,
            lambda numero: repository.numero_bl_disponible(numero, type_op))
        anomalies = [bl for bl in controles if bl["anomalie"]]
        valides = [bl for bl in controles if not bl["anomalie"]]

        k = st.columns(4)
        k[0].metric("Pages du PDF", total)
        k[1].metric("BL retenus", len(retenus))
        k[2].metric("Pages ignorées",
                    len(st.session_state.lot_ignores)
                    + max(0, total - st.session_state.lot_index))
        k[3].metric("À corriger", len(anomalies),
                    delta_color="inverse" if anomalies else "off")

        st.write(f"**{len(valides)} BL** seront enregistrés en "
                 f"« {repository.LIBELLES_OPERATION[type_op]} ».")
        st.dataframe(
            [{"Page": bl["page"] + 1, "N° BL": bl["numero"],
              "Date": bl["date_reception"], tiers: bl["fournisseur"],
              "Anomalie": bl["anomalie"] or "—"} for bl in controles],
            hide_index=True, width="stretch", height=min(420, 60 + 36 * len(controles)))

        if anomalies:
            st.warning(f"{len(anomalies)} ligne(s) en anomalie ne seront **pas** "
                       "enregistrées. Revenez à l'étape 3 pour les corriger, ou "
                       "validez le reste du lot.")

        col_prec, col_val = st.columns(2)
        if col_prec.button("⬅️ Revenir à la vérification", width="stretch"):
            st.session_state.lot_index = min(st.session_state.lot_index, total - 1)
            aller_a(3)
        if col_val.button(f"💾 Enregistrer les {len(valides)} BL", type="primary",
                          width="stretch", disabled=not valides):
            rbac.exiger_operation(type_op, CTX_RBAC)
            succes, echecs = [], []
            barre = st.progress(0.0, text="Enregistrement du lot…")
            for position, bl in enumerate(valides, start=1):
                try:
                    id_bl = str(uuid.uuid4())
                    repository.inserer_bl(
                        id_bl=id_bl, numero_bl=bl["numero"],
                        nom_fournisseur=bl["fournisseur"], statut_bl=repository.STATUT_OK,
                        type_operation=type_op, utilisateur=CURRENT_USER,
                        date_reception=bl["date_reception"])
                    repository.enregistrer_page(
                        id_bl, 0, st.session_state.lot_pages[bl["page"]])
                    repository.finaliser_bl(id_bl, 1, CURRENT_USER)
                    succes.append(bl["numero"])
                except Exception as exc:
                    # Un BL en échec n'annule pas les autres : chacun est une
                    # transaction indépendante, le détail est restitué ensuite.
                    logger.warning("Archivage du BL %s en échec : %s",
                                   bl["numero"], exc, exc_info=True)
                    echecs.append({"numero": bl["numero"],
                                   "motif": f"{type(exc).__name__} : {exc}"})
                barre.progress(position / len(valides),
                               text=f"Enregistrement {position}/{len(valides)}…")
            barre.empty()
            st.session_state.lot_resultats = {
                "succes": succes, "echecs": echecs,
                "ignores": len(st.session_state.lot_ignores),
                "anomalies": len(anomalies)}
            st.rerun()
    else:
        st.success(f"✅ **{len(resultats['succes'])} BL archivés** avec succès.")
        k = st.columns(3)
        k[0].metric("Enregistrés", len(resultats["succes"]))
        k[1].metric("En échec", len(resultats["echecs"]))
        k[2].metric("Non retenus", resultats["ignores"] + resultats["anomalies"])
        if resultats["echecs"]:
            st.error("Certains BL n'ont pas pu être enregistrés :")
            st.dataframe(resultats["echecs"], hide_index=True, width="stretch")
            st.caption("Les BL en échec peuvent être ressaisis en relançant un "
                       "lot avec les seules pages concernées.")
        with st.expander(f"Voir les {len(resultats['succes'])} numéros archivés"):
            st.write(", ".join(resultats["succes"]) or "—")
        if st.button("🆕 Archiver un nouveau lot", type="primary", width="stretch"):
