...
def injecter_style() -> None:
    ...
    st.markdown(
        ...
        /* Variante latérale : pastilles empilées sous le logo. La largeur de
           la barre étant contrainte, l'e-mail est tronqué proprement plutôt
           que de déborder (valeur complète en infobulle). */
        .bl-context-side {
            display:flex; flex-direction:column; gap:.3rem;
            margin:.1rem 0 .5rem;
        }
        .bl-context-side .bl-pill {
...
def barre_contexte_laterale(utilisateur: str, environnement: str,
                            roles: list[str]) -> None:
    """Même contexte, mais empilé dans la barre latérale (haut à gauche).

    Sur les écrans larges de l'app Administration, un bandeau horizontal en
    tête de page consomme une bande de hauteur utile sur *toutes* les vues.
    Placé à gauche, sous le logo, il ne coûte rien : la colonne existe déjà."""
    safe_user = html_lib.escape(utilisateur)
    safe_env = html_lib.escape(environnement.upper())
    role_text = ", ".join(map(html_lib.escape, roles)) or "Aucun rôle"
    st.markdown(
        '<div class="bl-context-side">'
